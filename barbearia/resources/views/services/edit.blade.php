                ...
                <form action="{{ route('services.update', $service) }}" method="POST" class="p-4 flex flex-col gap-4">
                    ...
                    <div>
                        ...
                        <textarea name="descrição" id="descrição" rows="3" 
                                  class="w-full text-sm p-2 border rounded-md bg-gray-50 focus:bg-white focus:outline-none @error('descrição') border-red-500 @else border-gray-300 @enderror">{{ old('descrição', $service->descrição) }}</textarea>
